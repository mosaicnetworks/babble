...
func TestJoinLateExtra(t *testing.T) {
	...
	initialNodes := initNodes(keys, peerSet, peerSet, 400, 400, 5, false, "badger", 30*time.Millisecond, logLevel, t)
	...
	newNode := newNode(peer, key, peerSet, peerSet, 400, 400, 5, false, "badger", 30*time.Millisecond, logLevel, t)
...
func TestSuccessiveJoinRequestExtra(t *testing.T) {
	...
	node0 := newNode(peerSet.Peers[0], keys[0], peerSet, genesisPeerSet, 10000, 400, 5, false, "inmem", 30*time.Millisecond, logLevel, t)
	...
	for i := 1; i <= 3; i++ {
		...
		newNode := newNode(peer, key, peerSet, genesisPeerSet, 1000, 400, 5, false, "inmem", 30*time.Millisecond, logLevel, t)
...
func TestSuccessiveLeaveRequestExtra(t *testing.T) {
	...
	nodes := initNodes(keys, peerSet, genesisPeerSet, 1000, 10000, 20, false, "inmem", 30*time.Millisecond, logLevel, t)
...
func TestSimultaneousLeaveRequestExtra(t *testing.T) {
	...
	nodes := initNodes(keys, peerSet, genesisPeerSet, 10000, 1000, 5, false, "inmem", 30*time.Millisecond, logLevel, t)
	defer shutdownNodes(nodes)
	//defer drawGraphs(nodes, t)
...
func TestJoinLeaveRequestExtra(t *testing.T) {
	...
	nodes := initNodes(keys, peerSet, genesisPeerSet, 1000, 10000, 5, false, "inmem", 30*time.Millisecond, logLevel, t)
	//defer drawGraphs(nodes, t)
	...
	newNode := newNode(peer, key, peerSet, genesisPeerSet, 10000, 200, 5, false, "inmem", 30*time.Millisecond, logLevel, t)
	defer newNode.Shutdown()
...
func TestJoiningAndLeavingExtra(t *testing.T) {
	...
	t.Log("Step 1")
	nodes01234 := initNodes(keys[0:5], peers01234, genesisPeerSet, 10000, 400, 15, false, "inmem", 30*time.Millisecond, logLevel, t) //make cache high to draw graphs
	...
	node5, nodes01235 := launchNodeAndGossip("Step 4 and 5", nodes0123, peerlist.Peers[5], keys[5],
		...
		"inmem", 30*time.Millisecond, t, 20, 8, true)
	...
	node6, nodes012356 := launchNodeAndGossip("Step 6 and 7", nodes01235, peerlist.Peers[6], keys[6],
		...
		"inmem", 30*time.Millisecond, t, 15, 8, true)
	...
	node7, nodes0123567 := launchNodeAndGossip("Step 8", nodes012356, peerlist.Peers[7], keys[7],
		...
		"inmem", 30*time.Millisecond, t, 14, 8, false)
	...
	node8, nodes01235678 := launchNodeAndGossip("Step 8b", nodes0123567, peerlist.Peers[8], keys[8],
		...
		"inmem", 30*time.Millisecond, t, 12, 8, false)
	...
	node9, nodes56789 := launchNodeAndGossip("Step 10", nodes5678, peerlist.Peers[9], keys[9],
		...
		"inmem", 30*time.Millisecond, t, 15, 8, false)
